...
if (isCookie("username")){
    ...
    if (window.location.href.split("/")[4] == "login") {
      ...
      document.querySelector(".login").insertAdjacentHTML("afterend", '<a class="home_icon" href="/finance/manage/"><img src="/finance/res/home-icon.svg"></a>');
    } else {
...
function check(username, hashedCredentials, cookie) {
  ...
  fetch("/finance/res/login.txt", {cache: "no-store"})
      ...
      for (var i = 0; i < credentialsList.length; i++) {
        ...
        //   if (!cookie) {
            // Set cookie
             ...
             document.cookie = "credentials=" + sha256(password.value) + ";path=/";
